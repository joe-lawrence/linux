"""
Test symbol visibility change: non-static (GLOBAL) to static (LOCAL).

klp_test_nts_func() starts as a non-static (globally visible) function.
The livepatch changes it to static. This tests objtool's ability to
correlate a GLOBAL original symbol with a LOCAL patched symbol - the
...
def generate_patches(test_dir, kernel_root, out_path):
    ...
    patch_file = Path(test_dir) / "lto-nonstatic-to-static.patch"
...
def verify_build(returncode, ko_path, results, tmp_dir=None, **kwargs):
    ...
    if not any("klp_test_visibility_proc_show" in n for n in old_names):
        raise VerificationError(
            "No klp_func.old_name contains 'klp_test_visibility_proc_show'"
        )
    results.append(
        "    VERIFIED: klp_func target contains 'klp_test_visibility_proc_show'"
    )
    ...
    if is_thinlto_config(config_path):
        ...
        suffixed = find_llvm_suffixed(vmlinux_path, "klp_test_nts_func")
        if suffixed:
            results.append(
                f"ThinLTO: original symbol promoted: {suffixed[0][0]}"
            )
        else:
            nm_type = get_symbol_nm_type(vmlinux_path, "klp_test_nts_func")
            if nm_type:
                results.append(
                    f"Original binding: {nm_type} (expected 'T' = GLOBAL)"
                )
    else:
        nm_type = get_symbol_nm_type(vmlinux_path, "klp_test_nts_func")
        if nm_type:
            results.append(
                f"Original binding: nm type '{nm_type}' (expected 'T' = GLOBAL)"
            )


def verify_runtime(runtime):
    """
    Verify that the livepatch applies despite the GLOBAL->LOCAL
    ...
    runtime.log("Non-static to static visibility runtime test")
    runtime.log("")
    ...
    cleanup_error = None
    try:
        result = runtime.read_sysfs("/proc/klp_test_visibility")
        ...
        runtime.log("  OK: function patched (GLOBAL->LOCAL correlation worked)")
    ...
    runtime.log("")
    runtime.log("SUCCESS: Non-static to static visibility test passed")
