...
def verify_build(returncode, ko_path, results, **kwargs):
    ...
    verify_klp_module_vmlinux(ko_path, results,
                              expected_funcs=["meminfo_proc_show",
                                              "cmdline_proc_show"],
                              tmp_dir=tmp_dir)
...
def verify_runtime(runtime):
    ...
    using diff -Nupr format. We trigger /proc/cmdline and /proc/meminfo
    ...
    cmdline_found = any("klp-build-test: add-remove-file-diff test" in msg for msg in messages)
