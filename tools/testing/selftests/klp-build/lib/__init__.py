...
def find_test_cases(category: str, speed: Optional[str] = None, test_type: Optional[str] = None) -> list[Path]:
    """
    Find all test case directories in a category (pass/fail).

    Args:
        category: "pass" or "fail"
        speed: Optional filter: "quick" or "long". Only applies to current-tree tests.
        test_type: Optional filter: "current-tree" or "patched-tree". If None, find both.
    ...
    # Determine which test types to search
    if test_type:
        test_types = [test_type]
    else:
        test_types = ["current-tree", "patched-tree"]

    for ttype in test_types:
        type_dir = tests_dir / ttype / category

        if not type_dir.exists():
            continue

        if ttype == "current-tree":
            # Current-tree has speed subdirectories (quick/long)
            if speed:
                # Search in specific speed category
                speed_dir = type_dir / speed
                if speed_dir.exists():
                ...
                for speed_cat in ["quick", "long"]:
                    speed_dir = type_dir / speed_cat
                    if speed_dir.exists():
                        for item in speed_dir.iterdir():
                            if item.is_dir() and (item / "expected.py").exists():
                                test_cases.append(item)
        else:
            # Patched-tree has tests directly under pass/fail
            for item in type_dir.iterdir():
                # Skip base/ directory (infrastructure, not a test)
                if item.name == "base":
                    continue
                if item.is_dir() and (item / "expected.py").exists():
                    test_cases.append(item)
