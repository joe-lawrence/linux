...
class RuntimeContext:
    ...
    def log(self, message: str) -> None:
        """Add a message to the verification log (for test commentary)."""
        self._verification_log.append(message)
    ...
    def disable(self) -> None:
        """Disable this livepatch module."""
        # Import here to avoid circular dependency
        disable_livepatch(self.mod_name)

    def unload(self) -> None:
        """Unload this livepatch module."""
        # Import here to avoid circular dependency
        unload_module(self.mod_name)
...
def unload_module(mod_name: str) -> None:
    ...
    """
    # Match upstream functions.sh unload_mod():
    # 1. Wait for refcnt == 0 (up to 60s)
    # 2. rmmod
    # 3. Wait for /sys/module/$mod to disappear (up to 60s)
    ...
    if refcnt_path.exists():
        ...
        while time.time() - start_time < 60:
            ...
            raise LivepatchError(
                f"Module {mod_name} refcnt did not reach 0 within 60s"
            )

    # Retry rmmod — after a livepatch is unloaded, the kernel may need a
    # moment to release internal references to patched modules.
    last_err = None
    start_time = time.time()
    while time.time() - start_time < 60:
        result = subprocess.run(
        ...
        raise LivepatchError(
            f"Failed to unload module {mod_name}: {last_err}"
        )
