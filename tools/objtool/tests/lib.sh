...
klp_preflight()
{
	...
	"$OBJTOOL" klp checksum "$tmp/probe.o" >/dev/null 2>&1 ||
		{ rm -rf "$tmp"
		  bail "objtool rejects an object built by '$CC'; they target different architectures (set CROSS_COMPILE, or --target= for clang)"; }
	...
	$OBJCOPY -O binary --only-section=.text "$tmp/probe.o" "$tmp/probe.bin" 2>/dev/null ||
		{ rm -rf "$tmp"
		  bail "$OBJCOPY cannot read objects built by '$CC'; install binutils-multiarch or set OBJCOPY=llvm-objcopy"; }
	...
	*)		host="$(uname -m)" ;;
	...
	[ -z "$cc_arch" ] || [ "$cc_arch" = "$arch" ] ||
		bail "ARCH says $arch but '$CC' builds $cc_arch objects; the $arch tests would run against the wrong architecture"
...
run_post_link()
{
	...
	[ "$rc" = "$expect" ] ||
		fail "klp post-link exited $rc, expected $expect: $(tail -2 "$workdir/post-link.log")"
}
...
# readelf prints as "OS [0xff20]".  The kernel resolves these itself at patch
# load; anything else is a symbol the module loader will try, and fail, to
# resolve normally.
assert_livepatch_sym()
{
	out_symbols | grep -F 'OS [0xff20]' | grep -qw -- "$1" ||
