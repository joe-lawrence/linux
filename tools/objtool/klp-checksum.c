				 ...
				 struct instruction *insn)
{
	...
	if (!reloc) {
		...
		if (call_dest || jump_dest) {
			...
			arch_jump_opcode_bytes(file, insn, buf, &len);
			checksum_update(func, insn, buf, len);
			...
			if (call_dest) {
				...
				/*
				 * Don't use insn_sym() here.  For alternative
				 * replacements, that would give the function
				 * of the code being replaced.
				 */
				dest_sym = jump_dest->_sym;
				if (!dest_sym)
					goto alts;

				checksum_update(func, insn, dest_sym->demangled_name,
